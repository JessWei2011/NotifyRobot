...
# 用途：設定平日 14:50 大盤法人金額、20:30 個股報告及每 10 分鐘事件警示
# ==============================================================================

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PYTHON_EXEC="$(which python3)"
LOG_FILE="$SCRIPT_DIR/bot.log"
MARKET_CRON_JOB="50 14 * * 1-5 cd $SCRIPT_DIR && $PYTHON_EXEC $SCRIPT_DIR/main.py --market-summary >> $LOG_FILE 2>&1"
...
echo "排程時間: 平日 14:50 大盤法人、20:30 個股報告"
...
# 新增排程
(crontab -l 2>/dev/null; echo "$MARKET_CRON_JOB"; echo "$STOCK_CRON_JOB"; echo "$EVENT_CRON_JOB"; echo "$CALENDAR_CRON_JOB") | crontab -
